...
```mermaid
flowchart TB
    subgraph RPi ["Raspberry Pi (常時稼働)"]
        direction TB
        
        subgraph UpdateProcess ["キャッシュ更新プロセス"]
            UpdateScript["update_cache.py<br/>10分間隔 / PC起動時のみ"]
            EPGAPI[("EPG Station API")]
            JSON[("reserves.json")]
        end

        subgraph WOLProcess ["WOL監視プロセス"]
            CheckScript["check_and_wol.py<br/>5分間隔 / 常時実行"]
            SendScript["send_wol.py<br/>WOLパケット送信"]
        end
    end

    subgraph PC ["デスクトップPC"]
        Desktop["PC本体<br/>スリープ/休止状態"]
    end

    %% Data Flow
    EPGAPI -->|予約情報取得| UpdateScript
    UpdateScript -->|保存| JSON
    JSON -->|予約チェック| CheckScript
    
    %% Logic Flow
    CheckScript -->|PC状態確認| Desktop
    CheckScript -->|25-30分前 or 0-5分前<br/>予約あり| SendScript
    SendScript -->|WOL送信| Desktop
    ...
    class UpdateScript,CheckScript,SendScript script;
    class JSON,EPGAPI storage;
